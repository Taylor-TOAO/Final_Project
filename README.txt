...
//A.h-D.h, include classes and derived classes which are called within the main file.
...
//to store data into their respective vectors. The files I have provided already have listed exaples however, if you wish to run your own, ensure that for each .txr file
...
//If the UN does not checkout or the password is either wrong or does not match the UN, the system will keep looping, allowing the user to input the correct information
...
//The main function calls outside functions within A.cpp. Within A.cpp is the class StockInvestmentCalculator and respective derived classes. A.cpp prompts the user with 
...
//The main function calls outside functions within B.cpp. Within B.cpp is the class InvestmentCalculator and the respective derived classes.
...
//The main function calls outside functions within C.cpp. Within C.cpp is the class MortgageCalculator and the respective derived classes.
...
//The derived class CalculateMortgage() then calculates the monthly mortgage payment
//The results are then sent/generated to an outfile "out.txt" for the user.
//The user is then asked if they would like to return to the main menu, if yes, the loop continues, if no the software exits.

//Option D: Collge Fund Investment Calculator
//The main function calls outside functions within D.cpp. Within D.cpp is the class MortgageCalculator and the respective derived classes.
//The first derived class getInput() receives user input for the principle amount (that is, the initial investment) and an expected interest rate of the investment.
//The second derived class readDataFromFile then takes monthly investments and stores them within the 'months' vector. The respective infile is OptD.txt and can contain 
//as many monthly investments as the user desires. The monthly investments are a dollar amount.
//The third derived class calculateCollegeFuncd() then calculates the investment based off the user input and monthly investments.
